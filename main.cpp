...
//  inky.update();
  ...
  bme68x.init();
    ...
    std::string message = " Temperatur: " + std::to_string(data.temperature) + "°C \n Feuchtigkeit: " +
                          ...
                          //std::to_string(data.status) + ", " +
                          //std::to_string(data.gas_index) + ", " +
                          //std::to_string(data.meas_index);
  

    char buffer[256];
    sprintf(buffer, "%lu, %.2f, %.2f, %.2f, %.2f, 0x%x, %d, %d\n",
        (long unsigned int)time_ms,
        data.temperature,
        data.pressure,
        data.humidity,
        ...
        data.gas_index,
        data.meas_index);
  std::string message_2(buffer);
  inky.text(message, {150, 450}, 400);
  inky.update();
  printf(buffer);
  printf("done!\n");
  sleep_ms(5000);
